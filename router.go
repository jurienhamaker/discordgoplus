...
type Router struct {
	...
	// Modals is a map of registered modal components
	// Key is the components custom ID. Value is the MessageComponent instance.
	//
	// NOTE: it is not recommended to use it directly, use RegisterMessageComponent, GetMessageComponent, UpdateMessageComponent, UnregisterMessageComponent functions instead.
...
// RegisterMessageComponent registers the message component.
func (r *Router) RegisterModal(cmpnt *Modal) {
	if _, ok := r.Modals[cmpnt.CustomID]; !ok {
		r.Modals[cmpnt.CustomID] = cmpnt
	}
}
...
func (r *Router) GetMessageComponent(
	customID string,
...
// GetModal returns a message component by specified CustomID.
// It also checks for params
func (r *Router) GetModal(
	customID string,
	...
	cmpnt, ok := r.Modals[customID]
	if ok {
		return cmpnt, nil
	}

	// do routing test & param parsing
	for _, cmpnt := range r.Modals {
		options, ok := cmpnt.try(customID)
		if ok {
			return cmpnt, options
		}
...
func (r *Router) UpdateMessageComponent(
	customID string,
	newcmpnt *MessageComponent,
) (cmd *MessageComponent, err error) {
...
// UpdateMessageComponent updates the message component and does all behind-the-scenes work.
func (r *Router) UpdateModal(
	customID string,
	newcmpnt *Modal,
) (cmd *Modal, err error) {
	if cmd, ok := r.Modals[customID]; ok {
		r.Modals[customID] = newcmpnt
		return cmd, nil
	}
...
func (r *Router) UnregisterMessageComponent(
	customID string,
) (messageComponent *MessageComponent, existed bool) {
...
// UnregisterModal removes a modal from router
func (r *Router) UnregisterModal(
	customID string,
) (modal *Modal, existed bool) {
	modal, existed = r.Modals[customID]

	if existed {
		delete(r.Modals, customID)
	}
...
// ListModals returns all registered modals
func (r *Router) ListModals() (list []*Modal) {
	if r == nil {
		return nil
	}

	for _, c := range r.Modals {
		list = append(list, c)
	}
	return
}
...
func (r *Router) getSubcommand(
	cmd *Command,
	opt *discordgo.ApplicationCommandInteractionDataOption,
	parent []Handler,
) (*Command, *discordgo.ApplicationCommandInteractionDataOption, []Handler) {
	if cmd == nil {
		return nil, nil, nil
	}
	...
	case discordgo.ApplicationCommandOptionSubCommand:
		return subcommand, opt, append(
			parent,
			append(subcommand.Middlewares, subcommand.Handler)...)
	case discordgo.ApplicationCommandOptionSubCommandGroup:
		return r.getSubcommand(
			subcommand,
			opt.Options[0],
			append(parent, subcommand.Middlewares...),
		)
	}
...
func (r *Router) HandleInteraction(
	s *discordgo.Session,
	...
	if len(data.Options) != 0 {
		cmd, parent, handlers = r.getSubcommand(
			cmd,
			data.Options[0],
			cmd.Middlewares,
		)
...
// HandleInteractionMessageComponent is an interaction handler passed to discordgo.Session.AddHandler.
func (r *Router) HandleInteractionMessageComponent(
	s *discordgo.Session,
	i *discordgo.InteractionCreate,
) {
...
// HandleInteractionModalSubmit is an interaction handler passed to discordgo.Session.AddHandler.
func (r *Router) HandleInteractionModalSubmit(
	s *discordgo.Session,
	i *discordgo.InteractionCreate,
) {
	...
	cmpnt, options := r.GetModal(data.CustomID)
	if cmpnt == nil {
		return
	}
	handlers := append(cmpnt.Middlewares, cmpnt.Handler)
...
func (r *Router) getMessageSubcommand(
	cmd *Command,
	arguments []string,
	...
	if subcommand != nil {
		if len(arguments) > 1 {
			return r.getMessageSubcommand(
				subcommand,
				arguments[1:],
				append(parent, subcommand.MessageMiddlewares...),
			) // TODO: opt-out
		} else {
			return subcommand, arguments[1:], append(
				parent,
				append(
					subcommand.MessageMiddlewares,
					subcommand.MessageHandler,
				)...)
		}
	}
	return cmd, arguments, append(parent, cmd.MessageHandler)
...
func (r *Router) MakeMessageHandler(
	...
	return func(s *discordgo.Session, m *discordgo.MessageCreate) {
		...
		command, arguments, handlers := r.getMessageSubcommand(
			command,
			arguments,
			command.MessageMiddlewares,
		)
...
func NewRouter(initial []*Command) (r *Router) {
	r = &Router{
		...
		Modals:            make(map[string]*Modal, 0),
