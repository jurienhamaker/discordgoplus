...
// Modal try customID on all stored message components
...
func (cmpnt Modal) try(customID string) (map[string]string, bool) {
	p := make(map[string]string, 0)
	var i, j int
	for i < len(customID) {
		switch {
		case j >= len(cmpnt.CustomID):
			if cmpnt.CustomID != "/" && len(cmpnt.CustomID) > 0 &&
				cmpnt.CustomID[len(cmpnt.CustomID)-1] == '/' {
				return nil, true
			}
			return nil, false
